...
if(isset($_POST['btn_action']))
{
	...
	{
		$query = "
		INSERT INTO inventory_order (user_id, inventory_order_total, inventory_order_date, inventory_order_name, payment_status, inventory_order_status, inventory_order_created_date) 
		VALUES (:user_id, :inventory_order_total, :inventory_order_date, :inventory_order_name, :payment_status, :inventory_order_status, :inventory_order_created_date)
		";
			...
			for($count = 0; $count<count($_POST["product_id"]); $count++)
			{
				...
				$statement->execute(
					array(
						...
						':price'				=>	$_POST["amount"][$count],
					)
				);
				$base_price = $_POST["amount"][$count] * $_POST["quantity"][$count];
		...
		foreach($sub_result as $sub_row)
		{
			...
			<span id="row'.$count.'">
				<div class="row">
					...
					<div class="col-md-3">
						<input type="text" name="amount[]" class="form-control" value="'.$sub_row["amount"].'" required />
					</div>
					<div class="col-md-3">
	...
	if($_POST['btn_action'] == 'Edit')
	{
		...
		if(isset($delete_result))
		{
			...
			for($count = 0; $count < count($_POST["product_id"]); $count++)
			{
				$product_details = fetch_product_details($_POST["product_id"][$count], $connect);
				$sub_query = "
				INSERT INTO inventory_order_product (inventory_order_id, product_id, quantity, price, tax) VALUES (:inventory_order_id, :product_id, :quantity, :price)
				";
				$statement = $connect->prepare($sub_query);
				$statement->execute(
					array(
				...
				$base_price = $product_details["price"][$count] * $_POST["quantity"][$count];
				$total_amount = $total_amount + $base_price;
			}
			$update_query = "
			UPDATE inventory_order 
